      ...
      <div className="sticky top-0 p-4 space-y-6">
        <form onSubmit={handleSearch} className="relative">
          <Input
            ...
            className="pl-9 bg-white/50 backdrop-blur-sm border-red-200 focus-visible:ring-red-200"
            value={searchQuery}
        ...
        <Accordion type="single" collapsible defaultValue="categories">
          <AccordionItem value="categories">
            <AccordionTrigger className="text-red-900 hover:text-red-700">Categories</AccordionTrigger>
            <AccordionContent>
              <nav className="space-y-1">
                {categories.map((category) => (
                  ...
                  >
                    {category.name}
                    <span className="ml-auto text-sm text-red-400">{category.count}</span>
                  </Button>
                ))}
              </nav>
            </AccordionContent>
